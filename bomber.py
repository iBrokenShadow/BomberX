...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("BomberX was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update BomberX.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/iBrokenShadow/BomberX.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/iBrokenShadow/BomberX.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/iBrokenShadow/BomberX/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("BomberX is up-to-date")
        mesgdcrt.GeneralMessage("Starting BomberX")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/iBrokenShadow/BomberX/raw/dev/.notify"
        else:
            url = "https://github.com/iBrokenShadow/BomberX/raw/master/.notify"
        noti = requests.get(url).text.upper()
        if len(noti) > 10:
            mesgdcrt.SectionMessage("NOTIFICATION: " + "For Any Queries, Join t.me/iBrokenShadow")
            print()
    except Exception:
        pass
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("Bomber is created by //Broken Shadow//")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("BomberX will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['//Broken Shadow//', 'iBrokenShadow',]
...
description = """BomberX - Your Friendly Spammer Application

BomberX can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

BomberX is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by Broken Shadow !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start BomberX with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start BomberX with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start BomberX with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update BomberX")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current BomberX contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current BomberX version")
